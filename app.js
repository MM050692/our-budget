...
let reportFrom = '';
let reportTo = '';
let activeStatementGraphModel = null;
let calendarMonth = '';
let toastTimer = null;
...
function renderStatement() {
  ...
  $('statementSummary').innerHTML = `<div class="statementRangeLabel"><span>${esc(rangeText)}</span><b>${allTransactions.length} ${allTransactions.length === 1 ? 'entry' : 'entries'}</b></div>
    <div class="statementTotals">
      <div class="income"><span>Money received</span><b>${baseMoney(incomeUSD)}</b></div>
      <div class="expense"><span>Money spent</span><b>${baseMoney(spentUSD)}</b></div>
      <div class="${netUSD < 0 ? 'expense' : 'net'}"><span>Net change</span><b>${signedBaseMoney(netUSD)}</b></div>
    </div>`;
...
function statementPeriodLabel(range) {
  const fullMonth = range.from.endsWith('-01') && range.to === addDays(addMonths(range.from, 1), -1);
  if (fullMonth) return formatDate(range.from, { month: 'long', year: 'numeric' });
  if (range.from === range.to) return formatDate(range.from, { day: 'numeric', month: 'long', year: 'numeric' });
  return `${formatDate(range.from, { day: 'numeric', month: 'short', year: 'numeric' })} – ${formatDate(range.to, { day: 'numeric', month: 'short', year: 'numeric' })}`;
}

function statementDailySeries(range, transactions) {
  if (dateDistance(range.from, range.to) > 61) return [];
  const byDate = new Map();
  transactions.forEach(transaction => {
    const day = byDate.get(transaction.date) || { incomeUSD: 0, spentUSD: 0, transactions: [] };
    const amountUSD = usd(transaction.amount, transaction.currency);
    if (transaction.type === 'income') day.incomeUSD += amountUSD;
    else day.spentUSD += amountUSD;
  ...
  for (let date = range.from; date <= range.to; date = addDays(date, 1)) {
    const saved = byDate.get(date) || { incomeUSD: 0, spentUSD: 0, transactions: [] };
    runningUSD += saved.incomeUSD - saved.spentUSD;
    days.push({ date, ...saved, runningUSD });
  }
  return days;
}

function statementSummaryModel(range = statementRange()) {
  const totals = statementReportTotals(range);
  const categoryMap = new Map();
  totals.transactions.filter(transaction => transaction.type === 'expense').forEach(transaction => {
    const category = transaction.category || 'Other';
    categoryMap.set(category, (categoryMap.get(category) || 0) + usd(transaction.amount, transaction.currency));
  });
  const allCategories = [...categoryMap].map(([label, amountUSD]) => ({
    label,
    amountUSD,
    share: totals.spentUSD > 0 ? amountUSD / totals.spentUSD : 0
  })).sort((left, right) => right.amountUSD - left.amountUSD);
  const categories = allCategories.slice(0, 5);
  if (allCategories.length > 5) {
    const amountUSD = allCategories.slice(5).reduce((sum, item) => sum + item.amountUSD, 0);
    categories.push({ label: 'Everything else', amountUSD, share: totals.spentUSD > 0 ? amountUSD / totals.spentUSD : 0 });
  }
  const hasRecords = totals.transactions.length > 0;
  const topCategory = allCategories[0];
  let status = 'even';
  let headline = 'Income and spending matched.';
  let explanation = 'Nothing was left from the money recorded for this period.';
  let nextAction = 'Choose one small amount to protect for Future before the next month starts.';
  if (!hasRecords) {
    status = 'empty';
    headline = 'There are no entries for this period yet.';
    explanation = 'Add money received and money spent to create the story for this period.';
    nextAction = 'Add the first income or spend, then open this summary again.';
  } else if (!(totals.incomeUSD > .005) && totals.spentUSD > .005) {
    status = 'missing';
    headline = 'Money received is missing.';
    explanation = `You recorded ${baseMoney(totals.spentUSD)} spent, but no income. If salary arrived in this period, add it before judging the result.`;
    nextAction = 'Add the missing salary or other income to the account that received it.';
  } else if (totals.differenceUSD > .005) {
    status = 'good';
    headline = `You kept ${baseMoney(totals.differenceUSD)} more than you spent.`;
    explanation = 'That is progress. The amount is already included in your account balances—do not enter it again as income.';
    nextAction = activeDebts().some(debt => debt.remaining > .005)
      ? `Agree together how much of the ${baseMoney(totals.differenceUSD)} will go to Debt and how much will go to Future.`
      : `Give the ${baseMoney(totals.differenceUSD)} one clear Future job: emergency savings, a goal or investing.`;
  } else if (totals.differenceUSD < -.005) {
    status = 'short';
    headline = `Spending was ${baseMoney(Math.abs(totals.differenceUSD))} higher than recorded income.`;
    explanation = `Check for missing income first.${topCategory ? ` If the records are complete, ${topCategory.label} was the biggest spending area.` : ''}`;
    nextAction = topCategory
      ? `Review ${topCategory.label} together and choose one small change for the next month.`
      : 'Check that every income entry was recorded for this period.';
  }
  const plan = [
    { pct: 40, label: 'Essentials', hint: 'home, food, transport and bills', targetUSD: totals.incomeUSD * .4, color: '#166b50' },
    { pct: 30, label: 'Debt', hint: 'minimums, then extra payoff', targetUSD: totals.incomeUSD * .3, color: '#654783' },
    { pct: 20, label: 'Future', hint: 'savings, goals and investing', targetUSD: totals.incomeUSD * .2, color: '#2878a8' },
    { pct: 10, label: 'Wants', hint: 'fun and flexible spending', targetUSD: totals.incomeUSD * .1, color: '#c96b59' }
  ];
  const days = statementDailySeries(range, totals.transactions);
  return { range, label: statementPeriodLabel(range), totals, categories, status, headline, explanation, nextAction, plan, days };
}

function statementGraphGeometry(days) {
  const width = 640;
  const height = 228;
  const pad = { left: 52, right: 18, top: 18, bottom: 34 };
  const values = days.map(day => day.runningUSD);
  let low = Math.min(0, ...values);
  let high = Math.max(0, ...values);
  if (Math.abs(high - low) < .01) { high += 1; low -= 1; }
  const plotWidth = width - pad.left - pad.right;
  const plotHeight = height - pad.top - pad.bottom;
  const x = index => pad.left + (days.length === 1 ? plotWidth / 2 : index * plotWidth / (days.length - 1));
  const y = value => pad.top + (high - value) * plotHeight / (high - low);
  return { width, height, pad, low, high, x, y, zeroY: y(0) };
}

function statementGraphMoney(value) {
  const converted = fromUSD(Math.abs(Number(value || 0)), state.settings.base);
  let amount = '';
  try { amount = new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(converted); }
  catch (_error) { amount = converted.toFixed(0); }
  return `${value < 0 ? '−' : ''}${state.settings.base} ${amount}`;
}

function statementGraphDayHtml(day) {
  const activity = day.transactions.slice().sort((left, right) => `${right.createdAt || ''}`.localeCompare(`${left.createdAt || ''}`));
  const items = activity.length
    ? `<div class="statementGraphItems">${activity.slice(0, 4).map(transaction => `<span class="${transaction.type}"><b>${esc(transaction.category)}</b><small>${transaction.type === 'income' ? '+' : '−'} ${money(transaction.amount, transaction.currency)}</small></span>`).join('')}${activity.length > 4 ? `<em>+ ${activity.length - 4} more ${activity.length - 4 === 1 ? 'entry' : 'entries'}</em>` : ''}</div>`
    : '<div class="statementGraphQuiet">No money received or spent on this day.</div>';
  return `<div class="statementGraphDayHead"><div><span>SELECTED DAY</span><b>${esc(formatDate(day.date, { weekday: 'short', day: 'numeric', month: 'long' }))}</b></div><strong class="${day.runningUSD < 0 ? 'negative' : ''}">${signedBaseMoney(day.runningUSD)}</strong></div>
    <div class="statementGraphDayTotals"><span><i class="income"></i>Received <b>${baseMoney(day.incomeUSD)}</b></span><span><i class="expense"></i>Spent <b>${baseMoney(day.spentUSD)}</b></span><span><i class="change"></i>Running change <b>${signedBaseMoney(day.runningUSD)}</b></span></div>${items}`;
}

function statementMonthGraphHtml(model) {
  if (!model.days.length) return `<section class="statementSimpleSection statementMonthGraph"><div class="statementSimpleHead"><div><b>Money through the period</b><span>Choose a month or up to 62 days for the daily graph</span></div></div></section>`;
  activeStatementGraphModel = model;
  const graphTitle = monthKey(model.range.from) === monthKey(model.range.to) ? 'How the month moved' : 'How this period moved';
  const geometry = statementGraphGeometry(model.days);
  const points = model.days.map((day, index) => ({ x: geometry.x(index), y: geometry.y(day.runningUSD), active: day.transactions.length > 0 }));
  const line = points.map((point, index) => `${index ? 'L' : 'M'}${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(' ');
  const area = `M${points[0].x.toFixed(2)},${geometry.zeroY.toFixed(2)} ${line} L${points.at(-1).x.toFixed(2)},${geometry.zeroY.toFixed(2)} Z`;
  const selectedIndex = model.days.length - 1;
  const selected = points[selectedIndex];
  const middleIndex = Math.floor((model.days.length - 1) / 2);
  return `<section class="statementSimpleSection statementMonthGraph"><div class="statementSimpleHead"><div><b>${graphTitle}</b><span>Tap or drag across any day</span></div><strong>Running change</strong></div>
    <div class="statementGraphPlot">
      <svg id="statementMonthGraphSvg" viewBox="0 0 ${geometry.width} ${geometry.height}" role="img" aria-label="Running monthly change by day" onpointerdown="selectStatementGraphFromPointer(event)" onpointermove="selectStatementGraphFromPointer(event, true)">
        <line class="statementGraphZero" x1="${geometry.pad.left}" x2="${geometry.width - geometry.pad.right}" y1="${geometry.zeroY.toFixed(2)}" y2="${geometry.zeroY.toFixed(2)}"></line>
        <path class="statementGraphArea" d="${area}"></path><path class="statementGraphLine" d="${line}"></path>
        ${points.map((point, index) => point.active ? `<circle class="statementGraphActivity${index === selectedIndex ? ' selected' : ''}" data-graph-index="${index}" cx="${point.x.toFixed(2)}" cy="${point.y.toFixed(2)}" r="4"></circle>` : '').join('')}
        <line id="statementGraphGuide" class="statementGraphGuide" x1="${selected.x.toFixed(2)}" x2="${selected.x.toFixed(2)}" y1="${geometry.pad.top}" y2="${geometry.height - geometry.pad.bottom}"></line>
        <circle id="statementGraphMarker" class="statementGraphMarker" cx="${selected.x.toFixed(2)}" cy="${selected.y.toFixed(2)}" r="7"></circle>
        <text class="statementGraphValue high" x="${geometry.pad.left}" y="12">${esc(statementGraphMoney(geometry.high))}</text>
        <text class="statementGraphValue low" x="${geometry.pad.left}" y="${geometry.height - 7}">${esc(statementGraphMoney(geometry.low))}</text>
        <text class="statementGraphDate first" x="${geometry.pad.left}" y="${geometry.height - 18}">${esc(formatDate(model.days[0].date, { day: 'numeric', month: 'short' }))}</text>
        <text class="statementGraphDate middle" x="${geometry.x(middleIndex).toFixed(2)}" y="${geometry.height - 18}">${esc(formatDate(model.days[middleIndex].date, { day: 'numeric', month: 'short' }))}</text>
        <text class="statementGraphDate last" x="${geometry.width - geometry.pad.right}" y="${geometry.height - 18}">${esc(formatDate(model.days.at(-1).date, { day: 'numeric', month: 'short' }))}</text>
      </svg>
      <input id="statementGraphSlider" class="statementGraphSlider" type="range" min="0" max="${selectedIndex}" value="${selectedIndex}" aria-label="Choose a day in the statement" oninput="selectStatementGraphPoint(this.value)">
    </div>
    <div id="statementGraphDetail" class="statementGraphDetail">${statementGraphDayHtml(model.days[selectedIndex])}</div>
    <details class="statementGraphHelp"><summary>How do I read this graph?</summary><p><b>Up</b> means more money came in than went out. <b>Down</b> means spending reduced the month’s result. <b>Flat</b> means no change. Tap a day to see what caused it. This is not your bank balance.</p></details>
  </section>`;
}

function selectStatementGraphPoint(value) {
  const days = activeStatementGraphModel?.days || [];
  if (!days.length) return;
  const index = Math.max(0, Math.min(days.length - 1, Math.round(Number(value) || 0)));
  const geometry = statementGraphGeometry(days);
  const day = days[index];
  const x = geometry.x(index);
  const y = geometry.y(day.runningUSD);
  $('statementGraphMarker')?.setAttribute('cx', x.toFixed(2));
  $('statementGraphMarker')?.setAttribute('cy', y.toFixed(2));
  $('statementGraphGuide')?.setAttribute('x1', x.toFixed(2));
  $('statementGraphGuide')?.setAttribute('x2', x.toFixed(2));
  if ($('statementGraphSlider')) $('statementGraphSlider').value = index;
  document.querySelectorAll('[data-graph-index]').forEach(point => point.classList.toggle('selected', Number(point.dataset.graphIndex) === index));
  if ($('statementGraphDetail')) $('statementGraphDetail').innerHTML = statementGraphDayHtml(day);
}

function selectStatementGraphFromPointer(event, moving = false) {
  if (moving && event.pointerType !== 'mouse' && !event.buttons) return;
  const days = activeStatementGraphModel?.days || [];
  const svg = event.currentTarget;
  if (!days.length || !svg) return;
  if (!moving) svg.setPointerCapture?.(event.pointerId);
  const rect = svg.getBoundingClientRect();
  const geometry = statementGraphGeometry(days);
  const scaledX = (event.clientX - rect.left) * geometry.width / Math.max(1, rect.width);
  const ratio = Math.max(0, Math.min(1, (scaledX - geometry.pad.left) / (geometry.width - geometry.pad.left - geometry.pad.right)));
  selectStatementGraphPoint(Math.round(ratio * (days.length - 1)));
}

function statementSummaryHtml(model) {
  const categoryRows = model.categories.length
    ? model.categories.map(item => `<div class="statementCategoryRow"><div><b>${esc(item.label)}</b><span>${Math.round(item.share * 100)}% of spending</span></div><strong>${baseMoney(item.amountUSD)}</strong><i style="--statement-bar:${Math.max(3, Math.round(item.share * 100))}%"></i></div>`).join('')
    : '<div class="statementSimpleEmpty">No spending was recorded for this period.</div>';
  return `<div class="statementSimple">
    <div class="statementStoryHero ${model.status}"><span>YOUR MONEY STORY</span><h3>${esc(model.label)}</h3><p>${esc(model.headline)}</p></div>
    <div class="statementEasyTotals">
      <div class="income"><span><b>Money received</b><small>Came in</small></span><strong>${baseMoney(model.totals.incomeUSD)}</strong></div>
      <div class="expense"><span><b>Money spent</b><small>Went out</small></span><strong>${baseMoney(model.totals.spentUSD)}</strong></div>
      <div class="${model.totals.differenceUSD < 0 ? 'expense' : 'change'}"><span><b>Net change</b><small>Received minus spent</small></span><strong>${signedBaseMoney(model.totals.differenceUSD)}</strong></div>
    </div>
    <div class="statementPlainAnswer"><b>What this means</b><p>${esc(model.explanation)}</p><small>Net change is not your bank balance. See Money for what is currently in each account.</small></div>
    ${statementMonthGraphHtml(model)}
    <section class="statementSimpleSection"><div class="statementSimpleHead"><div><b>Where money went</b><span>Largest area first</span></div><strong>${baseMoney(model.totals.spentUSD)}</strong></div><div class="statementCategoryList">${categoryRows}</div></section>
    <section class="statementSimpleSection"><div class="statementSimpleHead"><div><b>40–30–20–10 guide</b><span>A job for every bit of money received</span></div></div><div class="statementPlanGrid">${model.plan.map(item => `<div style="--plan-color:${item.color}"><span><b>${item.pct}%</b> ${esc(item.label)}</span><strong>${baseMoney(item.targetUSD)}</strong><small>${esc(item.hint)}</small></div>`).join('')}</div></section>
    <div class="statementNextAction"><span>ONE NEXT ACTION</span><b>${esc(model.nextAction)}</b></div>
    <details class="statementSakhiHelp"><summary>Sakhi's quick key</summary><p><b>पैसे आले</b> = money received · <b>खर्च</b> = money spent · <b>फरक</b> = the change during this period.</p></details>
  </div>`;
}
...
function statementCsvPackage(range = statementRange()) {
  ...
  const rows = [
    ['OUR DHAN · DETAILED RECORDS'],
    ['Period', statementPeriodLabel(range)],
    ['From', range.from, 'To', range.to],
    ['Currency used for the summary', dashboardCurrency],
    [],
    ['MONTH AT A GLANCE'],
    ['Money received', Number(fromUSD(totals.incomeUSD, dashboardCurrency).toFixed(2)), dashboardCurrency],
    ['Money spent', Number(fromUSD(totals.spentUSD, dashboardCurrency).toFixed(2)), dashboardCurrency],
    ['Net change (received minus spent)', Number(fromUSD(totals.differenceUSD, dashboardCurrency).toFixed(2)), dashboardCurrency],
    ['Important', 'Net change is not your bank balance. Open Money in Our DHAN to see current account balances.'],
    ...(!(totals.incomeUSD > .005) && totals.spentUSD > .005 ? [['Check this month', 'No income was recorded. Add salary or other income if it was received during this period.']] : []),
    [],
    ['DETAILED ENTRIES · kept for backup and checking'],
    ['Date', 'Money in or out', 'What was it for', 'Original amount', 'Currency', `Value in ${dashboardCurrency}`, 'Who', 'Account', 'Note'],
    ...[...totals.transactions].reverse().map(transaction => [
      transaction.date, transaction.type === 'income' ? 'Money in' : 'Money out', transaction.category, transaction.amount, transaction.currency,
      Number(fromUSD(usd(transaction.amount, transaction.currency), dashboardCurrency).toFixed(2)),
...
function renderStatementGenerator() {
  ...
  const model = statementSummaryModel(range);
  ...
  root.innerHTML = `<div class="statementGenerator">
    <p>Choose a period. The simple summary is made for reading and sharing; detailed records remain available when you need them.</p>
...
function statementSummaryFilename(range) { return `our-dhan-simple-summary-${range.from}-to-${range.to}.png`; }

function canvasRoundedRect(context, x, y, width, height, radius, fill) {
...
function canvasFittedText(context, text, x, y, maxWidth, startSize = 40, minSize = 25) {
  let size = startSize;
  do { context.font = `750 ${size}px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`; size -= 1; }
  while (size >= minSize && context.measureText(text).width > maxWidth);
  context.fillText(text, x, y);
}

function statementSummaryCanvas(range) {
  const model = statementSummaryModel(range);
  ...
  const heroColor = ['short', 'missing'].includes(model.status) ? '#71473f' : model.status === 'good' ? '#175c49' : '#49365f';
  canvasRoundedRect(context, 56, 54, 968, 250, 38, heroColor);
  context.fillStyle = 'rgba(255,255,255,.7)';
  context.font = '700 22px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.letterSpacing = '2px';
  context.fillText('OUR DHAN · YOUR MONEY STORY', 94, 102);
  context.letterSpacing = '0px';
  context.fillStyle = '#ffffff';
  context.font = '760 58px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.fillText(model.label, 94, 177);
  context.fillStyle = 'rgba(255,255,255,.84)';
  context.font = '500 29px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  canvasWrappedText(context, model.headline, 94, 229, 870, 38, 2);

  const totalCards = [
    { label: 'MONEY RECEIVED', hint: 'Came in', value: baseMoney(model.totals.incomeUSD), color: green },
    { label: 'MONEY SPENT', hint: 'Went out', value: baseMoney(model.totals.spentUSD), color: coral },
    { label: 'NET CHANGE', hint: 'Received minus spent', value: signedBaseMoney(model.totals.differenceUSD), color: purple }
  ];
  totalCards.forEach((card, index) => {
    const x = 56 + index * 327;
    canvasRoundedRect(context, x, 330, 307, 170, 28, '#ffffff');
    context.fillStyle = card.color;
    context.font = '750 19px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    context.fillText(card.label, x + 28, 373);
    context.fillStyle = ink;
    canvasFittedText(context, card.value, x + 28, 427, 251, 36, 23);
    context.fillStyle = muted;
    context.font = '500 18px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    context.fillText(card.hint, x + 28, 469);
  });

  canvasRoundedRect(context, 56, 526, 968, 344, 32, '#ffffff');
  context.fillStyle = ink;
  context.font = '740 30px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.fillText(monthKey(range.from) === monthKey(range.to) ? 'How the month moved' : 'How this period moved', 88, 575);
  context.fillStyle = muted;
  context.font = '500 19px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.fillText('The line is the running change—not your bank balance.', 88, 609);
  if (model.days.length) {
    const chart = { x: 105, y: 638, width: 868, height: 172 };
    const values = model.days.map(day => day.runningUSD);
    let low = Math.min(0, ...values);
    let high = Math.max(0, ...values);
    if (Math.abs(high - low) < .01) { high += 1; low -= 1; }
    const pointX = index => chart.x + (model.days.length === 1 ? chart.width / 2 : index * chart.width / (model.days.length - 1));
    const pointY = value => chart.y + (high - value) * chart.height / (high - low);
    const zeroY = pointY(0);
    context.strokeStyle = '#d8d6dd';
    context.setLineDash([10, 10]);
    context.beginPath(); context.moveTo(chart.x, zeroY); context.lineTo(chart.x + chart.width, zeroY); context.stroke();
    context.setLineDash([]);
    context.beginPath();
    model.days.forEach((day, index) => {
      const x = pointX(index); const y = pointY(day.runningUSD);
    ...
    context.stroke();
    model.days.forEach((day, index) => {
      if (!day.transactions.length) return;
      context.beginPath(); context.arc(pointX(index), pointY(day.runningUSD), 7, 0, Math.PI * 2);
      context.fillStyle = '#ffffff'; context.fill(); context.lineWidth = 5; context.strokeStyle = purple; context.stroke();
    });
    context.fillStyle = muted;
    context.font = '600 17px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    context.textAlign = 'left'; context.fillText(formatDate(model.days[0].date, { day: 'numeric', month: 'short' }), chart.x, 844);
    context.textAlign = 'center'; context.fillText(formatDate(model.days[Math.floor((model.days.length - 1) / 2)].date, { day: 'numeric', month: 'short' }), chart.x + chart.width / 2, 844);
    context.textAlign = 'right'; context.fillText(formatDate(model.days[model.days.length - 1].date, { day: 'numeric', month: 'short' }), chart.x + chart.width, 844);
    context.textAlign = 'left';
  }

  const categoryHeight = 116 + Math.max(1, model.categories.length) * 62;
  canvasRoundedRect(context, 56, 896, 968, categoryHeight, 32, '#ffffff');
  context.fillStyle = ink;
  context.font = '740 30px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.fillText('Where money went', 88, 947);
  context.fillStyle = muted;
  context.font = '500 18px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.fillText('Largest area first', 88, 978);
  if (model.categories.length) {
    model.categories.forEach((item, index) => {
      const y = 1020 + index * 62;
      context.fillStyle = ink; context.font = '650 21px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText(item.label, 88, y);
      context.textAlign = 'right'; context.fillText(baseMoney(item.amountUSD), 986, y); context.textAlign = 'left';
      canvasRoundedRect(context, 88, y + 17, 898, 10, 5, '#eceaf0');
      canvasRoundedRect(context, 88, y + 17, Math.max(12, 898 * item.share), 10, 5, purple);
    });
  } else {
    context.fillStyle = muted; context.font = '500 21px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText('No spending was recorded.', 88, 1030);
  }

  const planY = 896 + categoryHeight + 24;
  canvasRoundedRect(context, 56, planY, 968, 334, 32, '#ffffff');
  context.fillStyle = ink; context.font = '740 30px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText('40–30–20–10 guide', 88, planY + 51);
  context.fillStyle = muted; context.font = '500 18px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText('A simple job for all money received', 88, planY + 82);
  model.plan.forEach((item, index) => {
    const column = index % 2;
    const row = Math.floor(index / 2);
    const x = 88 + column * 454;
    const y = planY + 110 + row * 98;
    canvasRoundedRect(context, x, y, 426, 78, 19, '#f5f4f7');
    canvasRoundedRect(context, x, y, 9, 78, 5, item.color);
    context.fillStyle = item.color; context.font = '760 22px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText(`${item.pct}% ${item.label}`, x + 26, y + 31);
    context.fillStyle = ink; context.font = '680 23px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.textAlign = 'right'; context.fillText(baseMoney(item.targetUSD), x + 404, y + 31); context.textAlign = 'left';
    context.fillStyle = muted; context.font = '500 16px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText(item.hint, x + 26, y + 59);
  });

  const actionY = planY + 358;
  canvasRoundedRect(context, 56, actionY, 968, 170, 32, '#eee7f5');
  context.fillStyle = purple; context.font = '780 19px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'; context.fillText('ONE NEXT ACTION', 88, actionY + 44);
  context.fillStyle = ink; context.font = '680 28px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  canvasWrappedText(context, model.nextAction, 88, actionY + 88, 890, 38, 2);
  context.fillStyle = muted; context.font = '500 17px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  context.fillText('Made privately on your phone · Detailed records stay in Our DHAN', 56, canvas.height - 30);
  return canvas;
}
...
function canvasPngBlob(canvas) {
  return new Promise((resolve, reject) => {
    if (canvas.toBlob) {
      canvas.toBlob(blob => blob ? resolve(blob) : reject(new Error('Picture could not be created')), 'image/png');
      return;
    }
    try {
...
async function statementSummaryBlob(range) { return canvasPngBlob(statementSummaryCanvas(range)); }
...
async function shareStatementSummary(range, preparedPeriodStart = '') {
  const model = statementSummaryModel(range);
  const label = model.label;
  let blob = null;
  try { blob = await statementSummaryBlob(range); }
  catch (_error) { toast('The summary picture could not be created. Your records are safe.'); return false; }
  const filename = statementSummaryFilename(range);
  let file = null;
  try { file = new File([blob], filename, { type: 'image/png' }); }
  catch (_error) { file = null; }
  const summary = preparedStatementEmailText(range, model.totals);
  ...
  if (canShareFile) {
    try {
      ...
      if (preparedPeriodStart) markPreparedStatementHandled(preparedPeriodStart, 'shared');
      closeModal();
      toast('Simple summary shared ✓');
      return true;
    } catch (error) {
      if (error?.name !== 'AbortError') toast('Sharing did not open. Use Save picture instead.');
      return false;
    }
  }
  downloadBlobFile(filename, blob);
  if (preparedPeriodStart) markPreparedStatementHandled(preparedPeriodStart, 'email_opened');
  const recipient = state.member.role === 'owner' ? currentUser?.email || '' : '';
  const draftBody = 'Your simple Our DHAN summary picture was saved on this device. Attach the PNG picture to this draft, then send it to yourself.';
  location.href = `mailto:${encodeURIComponent(recipient)}?subject=${encodeURIComponent(`Our DHAN · ${label} summary`)}&body=${encodeURIComponent(draftBody)}`;
  toast('Summary picture saved · email draft opened');
  return true;
}

async function downloadStatementSummary(range, preparedPeriodStart = '') {
  try {
    const blob = await statementSummaryBlob(range);
    downloadBlobFile(statementSummaryFilename(range), blob);
    if (preparedPeriodStart) markPreparedStatementHandled(preparedPeriodStart, 'picture_downloaded');
    toast('Simple summary picture saved ✓');
  } catch (_error) { toast('The summary picture could not be created. Your records are safe.'); }
}

function shareGeneratedStatement() { return shareStatementSummary(reportRange()); }
...
function preparedStatementEmailText(range, totals) {
  ...
  return `Our DHAN · ${label}\n\nMoney received: ${baseMoney(totals.incomeUSD)}\nMoney spent: ${baseMoney(totals.spentUSD)}\nNet change: ${signedBaseMoney(totals.differenceUSD)}\n\nNet change means received minus spent; it is not the bank balance. The attached picture includes the daily graph, spending areas, 40–30–20–10 guide and one next action. Transfers and balance corrections are excluded.`;
}
...
function openPreparedMonthlyStatement(periodStart = '') {
  ...
  const label = formatDate(range.from, { month: 'long', year: 'numeric' });
  const model = statementSummaryModel(range);
  openModal(`${label} money story`, `<div class="statementGenerator preparedStatement">
    <p>Prepared automatically from your saved records. The easy summary is created on this device and no statement file is stored online.</p>
    ${statementSummaryHtml(model)}
    <div class="friendlyNote">Share sends the easy picture through your phone. The detailed CSV is optional and intended for backup or checking.</div>
    <div class="buttonRow statementShareActions"><button type="button" class="secondary" onclick="downloadPreparedMonthlyStatement('${range.from}')">Detailed CSV</button><button type="button" class="secondary" onclick="downloadPreparedMonthlySummary('${range.from}')">Save picture</button><button type="button" class="primary" onclick="sharePreparedMonthlyStatement('${range.from}')">Share simple summary</button></div>
  </div>`);
}
...
function downloadPreparedMonthlySummary(periodStart) {
  const prepared = preparedStatementFor(periodStart);
  return downloadStatementSummary(prepared.range, prepared.range.from);
}

async function sharePreparedMonthlyStatement(periodStart) {
  const prepared = preparedStatementFor(periodStart);
  return shareStatementSummary(prepared.range, prepared.range.from);
}
...
function renderStatementEmailSettings() {
  ...
  root.innerHTML = ready
    ? `<h3>Monthly money story ready</h3><p>${esc(label)} · easy picture, daily graph and optional detailed records.</p>`
    : latest
...
function openStatementEmailInfo() {
  openModal('Monthly statement sharing', `<div class="emailInfo">
    ...
    <div class="emailInfoSteps">
      ...
      <div><i>2</i><span><b>Previous month becomes a story</b><small>See money received, money spent, the daily graph, spending areas and the 40–30–20–10 guide.</small></span></div>
      <div><i>3</i><span><b>Share the easy picture</b><small>Your phone opens Mail, Gmail or another app with the simple summary picture attached when supported.</small></span></div>
    </div>
...
function csvCell(value) {
  ...
  const numeric = /^\s*[+-]?(?:\d+(?:\.\d*)?|\.\d+)\s*$/.test(text);
  const safe = !numeric && /^\s*[=+\-@]/.test(text) ? `'${text}` : text;
